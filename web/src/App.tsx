...
function AppInner() {
  ...
  const addItems = (itemsToAdd: ZoteroItem[]) => {
    ...
    let nextNodes = [...flowNodes]; let paperCount = nextNodes.filter(node => node.type === 'paperCard').length; const addedKeys: string[] = [];
    for (const item of itemsToAdd) {
      if (nextNodes.some(node => node.type === 'paperCard' && (node.data as PaperNodeData).itemKey === item.key)) { addedKeys.push(item.key); continue; }
    ...
    if (nextNodes !== flowNodes) recordHistory(nextNodes, flowEdges);
  ...
  const saveDialog = () => {
    ...
    const edge: LitFlowEdge = { id: `edge:${crypto.randomUUID()}`, source: dialog.source, target: dialog.target, label: dialog.label || '', type: 'smoothstep', markerEnd: dialog.directed ? { type: MarkerType.ArrowClosed, color: dialog.color } : undefined, style: { stroke: dialog.color, strokeWidth: 2, strokeDasharray: dialog.lineStyle === 'dashed' ? '8 5' : dialog.lineStyle === 'dotted' ? '2 6' : undefined }, labelStyle: { fill: dialog.color, fontWeight: 700 }, data: { note: dialog.note, color: dialog.color, isDirected: dialog.directed } };
    const nextEdges = addEdge(edge, flowEdges); recordHistory(flowNodes, nextEdges); setDialog(null);
  };
